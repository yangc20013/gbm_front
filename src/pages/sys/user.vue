    ...
    <div slot="body">
      <el-table
        ...
        <el-table-column
          label="状态">
          <template slot-scope="scope">
            {{ scope.row.enabled===true ? '已激活' : '未激活' }}
          </template>
        </el-table-column>
    ...
    methods: {
      ...
      handleRoleConfig(index, row){
        if(row.username == 'admin'){
          this.$message.error('不能配置系统管理员的角色');
          return;
        }
        this.currentRow = row;
      ...
      configUserRoles(){
        let checkedKeys = this.$refs.roleTree.getCheckedKeys();
          this.$http.post(api.SYS_SET_USER_ROLE + "?userId=" + this.currentRow.id + "&roleIds="+checkedKeys.join(','))
          .then(res => {
              this.$message('修改成功');
      ...
      handleDelete(index, row){
        ...
        this.$http.delete(api.SYS_USER_DELETE + "?id=" + row.id).then(res => {
          this.loadData();
        });
      },
      loadData(){
          ...
          .then(res => {
            this.tableData.rows = res.content;
            this.tableData.pagination.total = res.totalElements;
          });
      }
    },
    created(){
